    ...
    <div className="pt-28 pb-20 px-4 sm:px-6 lg:px-40 animate-in fade-in duration-500">
      <div className="max-w-[1440px] mx-auto space-y-12 md:space-y-16">
        <div className="flex flex-col gap-3 md:gap-4 text-center">
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black font-serif text-white tracking-tight">{t.contact.title} <span className="text-primary/80">[JP TIRES]</span>
          </h1>
          <p className="text-white text-lg md:text-xl max-w-2xl mx-auto italic">{t.contact.subtitle}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16 lg:gap-20">
          <div className="space-y-8 md:space-y-12">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
              <a 
                ...
                className="group p-6 md:p-8 rounded-xl md:rounded-2xl bg-surface-dark/35 border border-white/10 hover:border-primary/50 transition-all flex flex-col justify-between min-h-[140px]"
              >
                <div className="flex items-center gap-3 md:gap-4 text-primary mb-4 md:mb-6">
                  ...
                  <span className="text-xs md:text-sm uppercase font-black tracking-widest">{t.contact.callUs}</span>
                </div>
                <p className="text-white text-xl md:text-2xl font-bold group-hover:text-primary transition-colors">+1 438-826-6657</p>
              </a>
              <a 
              ...
              >
                <div className="flex items-center gap-3 md:gap-4 text-primary mb-4 md:mb-6">
                  ...
                  <span className="text-xs md:text-sm uppercase font-black tracking-widest">{t.contact.email}</span>
                </div>
                <p className="text-white text-lg md:text-xl font-bold group-hover:text-primary transition-colors truncate">Jean_plourde@hotmail.com</p>
              </a>
            </div>

            <div className="space-y-8 md:space-y-10 border-t border-white/10 pt-8 md:pt-10">
              <div className="flex flex-col gap-3 md:gap-4">
                <h4 className="text-primary text-xs md:text-sm uppercase font-black tracking-[0.25em] md:tracking-[0.3em]">{t.contact.showroom}</h4>
                <p className="text-white text-xl sm:text-2xl md:text-3xl font-serif leading-relaxed">{mapAddress}</p>
              </div>
              <div className="flex flex-col gap-3 md:gap-4">
                <h4 className="text-primary text-xs md:text-sm uppercase font-black tracking-[0.25em] md:tracking-[0.3em]">{t.contact.hours}</h4>
            ...
            <div className="flex flex-col gap-6 md:gap-8">
              <h3 className="text-primary text-xs md:text-sm uppercase font-black tracking-[0.25em] md:tracking-[0.3em]">{t.contact.paymentMethods}</h3>
              <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 md:gap-4">
                {paymentMethods.map((method, i) => (
                   <div key={i} className="flex flex-col items-center gap-2 md:gap-3 group">
                     ...
                     <span className="text-[9px] md:text-[10px] text-white uppercase font-black tracking-widest text-center">
                       {method.name}
                     </span>
                   </div>
                ))}
          ...
          <div className="relative min-h-[400px] md:min-h-[500px] h-full rounded-[2rem] md:rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl group">
